...
final class CodexConfigEditorTests: XCTestCase {
    ...
    // MARK: - 切换 active Provider（联动 reapply）

    func testEnableWithTargetProviderSwitchesModelProviderAndSection() throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let configURL = directory.appendingPathComponent("config.toml")
        ...
        let editor = CodexConfigEditor()
        let configuration = try editor.enable(
            at: configURL, port: 17891, bridgeModel: nil,
            providerName: "other", upstreamBaseURL: "https://other.example/v1"
        )
        XCTAssertEqual(configuration.providerName, "other")
        XCTAssertEqual(configuration.upstreamBaseURL, "https://other.example/v1")
        XCTAssertEqual(configuration.localBaseURL, "http://127.0.0.1:17891/v1")
        XCTAssertFalse(configuration.providerSectionInjected)
        let content = try String(contentsOf: configURL, encoding: .utf8)
        XCTAssertTrue(content.contains("model_provider = \"other\""))
        XCTAssertTrue(content.contains("base_url = \"http://127.0.0.1:17891/v1\""))
        // 旧 relay 段不动（restore 阶段负责还原其 base_url）。
        XCTAssertTrue(content.contains("[model_providers.relay]"))
    }

    func testEnableWithTargetProviderInjectsSectionWhenMissing() throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let configURL = directory.appendingPathComponent("config.toml")
        ...
        let configuration = try editor.enable(
            at: configURL, port: 17891, bridgeModel: nil,
            providerName: "newone", upstreamBaseURL: "https://new.example/api"
        )
        XCTAssertEqual(configuration.providerName, "newone")
        XCTAssertTrue(configuration.providerSectionInjected) // 段是新注入的
        let content = try String(contentsOf: configURL, encoding: .utf8)
        XCTAssertTrue(content.contains("model_provider = \"newone\""))
        XCTAssertTrue(content.contains("[model_providers.newone]"))
        XCTAssertTrue(content.contains("base_url = \"http://127.0.0.1:17891/api\""))
    }
}
